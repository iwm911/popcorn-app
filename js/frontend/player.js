...
var playTorrent = window.playTorrent = function(torrent, subs, tvModel, movieModel, callback, progressCallback) {
        ...
        flix.server.on('listening', function() {
            ...
            var checkLoadingProgress = function() {
                ...
                var now = flix.downloaded,
                    ...
                    // Some movies & tv shows need quite a few frames to play properly, or else the user gets another (shittier) loading screen on the video player.
                ...
                if (now > targetLoaded) {
                    if (typeof window.spawnVideoPlayer === 'function') {
                        window.spawnVideoPlayer(href, subs, tvModel, movieModel);
                    }
...
window.spawnVideoPlayer = function(url, subs, tvModel, movieModel) {

    // Sort sub according lang translation
    var subArray = [];
    for (var lang in subs) {
        if (typeof SubtitleLanguages[lang] == 'undefined') {
            continue;
        }
        subArray.push({
            'language': lang,
    ...
    // Init video.
    var video = window.videoPlaying = videojs('video_player', {
        plugins: {
            biggerSubtitle: {},
            smallerSubtitle: {},
            customSubtitles: {}
